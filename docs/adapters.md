...
The Go adapter acquires syntax-local facts for packages, named types, structs and interfaces, fields and struct tags, constants and variables, functions and methods, receivers and parameters, type expressions, calls, conditionals, switches, loops, returns, panic calls, comments, and common `testing.T` failure calls. Every emitted fact carries caller-supplied source identity and revision together with exact byte/row/column spans. Ordinary `.go` files are selected by the shared runtime and therefore enter the same public `chirograph analyze` acquisition path as other supported sources.

The adapter is intentionally not a Go type checker, module resolver, build-system interpreter, package loader, or runtime. It does not execute Go, invoke `go` tooling, download modules, follow imports, resolve identifiers across files, or infer repository-specific contract meaning. A filename, generated-looking path, or ordinary comment does not establish generated-from lineage; generation relationships require explicit mechanically grounded evidence at a higher layer. Kubernetes and Temporal are pinned acceptance specimens only and are not recognized by production code.
